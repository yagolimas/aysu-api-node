...
class Session {
    ...
    setRatePerceivedExertion(ratePerceivedExertion) {
     ...
     *   2.1 - Calcular m�dia das m�dias do intervalos
    ...
    getStandardDeviationOfNN() {
        ...
        const meanRRIntervals = meansRRIntervals.reduce((total, meanValueRR) => total + meanValueRR / meansRRIntervals.length, 0);
        const varianceRRIntervals = meansRRIntervals.reduce((total, meanValueRR) => total + Math.pow(meanRRIntervals - meanValueRR, 2) / (meansRRIntervals.length - 1), 0);
